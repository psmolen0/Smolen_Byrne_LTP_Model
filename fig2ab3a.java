...
public class fig2ab3a

/* To compile and run these supplemental java programs, install Java Development Kit (JDK) from Oracle, or a similar compiler. Rename this file as fig2ab3a.java. With JDK, type “javac fig2ab3a.java” to compile and “java fig2ab3a” to run. */
...
/* The following timing parameters can be adjusted as needed to reproduce simulations in Figs. 2A, 2B, or 3A */

double recstart=3000.0; // Time to start writing data

    double stime=recstart+179.999; // Time of stimulus onset

    double isi=5.0;      // Spacing between tetanic stimuli

    double recend=stime+899.999; 
...
// pkm level, and phosphorylation sites on TFs for CaMKII and ERK
	...
	double perk;
	double pkms;
	double pkmfterm;  // term for PKM autoactivation
...
// PKM parameters
	...
	double epsilpkm=0.0; /* coupling constant to tune the PKM positive feedback strength, I find that 0.028 is the number I want for bistability here (the equations are simplified and modified from the 2012 paper, so easier to get bistability) */

	double epsilck2=0.0; // coupling constant to tune strength of CaMKII autoactivation positive feedback loop.  1.0 gives strong bistability.

	double kfeedck2=4.0; // two more parameters for quadratic CaMKII feedback loop
	double Khck2=1.0;
...
/* for CaMKII feedback simulation, same values except kltp is adjusted down to 70. */

/* For PKM only feedback simulation, need kltp back up. Is up to 300. */

/* kltp = 100 is standard value for generating bistability in W with the upper state stabilized by synaptic reactivation. */

	double kltp=500.0; 

	double vsynbas = 0.01; 

	double tausyn=300.0; // 300 is standard value.
...
// INNER SIMULATION LOOP (SMALLER TIMESTEP DT)
	...
	/* code for inhibition simulation	tref=time-inhtime; */
...
// DURING THE TETANIC PROTOCOL. 

/* note that several LTP stimulus protocols, including chem-LTP and theta-LTP, can be simulated here. See Smolen et al 2006 for these other protocols */
...
dv1dt = kfck2*(powca/(powca+powkc)) - ck2act/tauck2 + epsilck2*(kfeedck2/tauck2*ck2act*ck2act/(Khck2*Khck2+ck2act*ck2act));
...
pkmfterm = pkms*pkms/(pkms*pkms+Kpkm*Kpkm);

dv16dt = ktranspkm1*pck2*perk + vbaspkm + epsilpkm*pkmfterm - kdegpkm*pkms;
